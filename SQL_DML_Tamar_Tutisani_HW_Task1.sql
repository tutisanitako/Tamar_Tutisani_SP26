...
INSERT...SELECT advantage over INSERT...VALUES:
	language_id is resolved at runtime via SELECT from the language
	table rather than being hardcoded. This makes the script
	portable across DB instances and safe against ID drift over time.

REFERENTIAL INTEGRITY:
	The language_id FK is always satisfied because it is derived
	from an actual row in the language table. 
...
-- Film 1: 13 Going on 30 (2004)  rental_rate = 4.99 | rental_duration = 7 days (1 week)
...
SELECT
    '13 Going on 30' AS title,
    'A 13-year-old girl magically wakes up as a 30-year-old woman in Manhattan.' AS description,
...
WHERE UPPER(lang.name) = 'ENGLISH' AND
      NOT EXISTS (
          SELECT 1
          FROM public.film f
          WHERE UPPER(f.title) = UPPER('13 Going on 30')
      )
RETURNING film_id, title, description, release_year, language_id, rental_duration, 
		  rental_rate, length, replacement_cost, rating, last_update;


-- Film 2: Coherence (2013)  rental_rate = 9.99 | rental_duration = 14 days (2 weeks)
...
SELECT
	'Coherence' AS title,
	'Strange phenomena unfold for a group of friends at a dinner party during a comet pass.' AS description,
...
WHERE UPPER(lang.name) = 'ENGLISH' AND
	  NOT EXISTS (
		  SELECT 1
		  FROM public.film f
		  WHERE UPPER(f.title) = UPPER('COHERENCE')
	  )
RETURNING film_id, title, description, release_year, language_id, rental_duration, 
		  rental_rate, length, replacement_cost, rating, last_update;

-- Film 3: The Prestige (2006)  rental_rate = 19.99 | rental_duration = 21 days (3 weeks)
INSERT INTO public.film(
	title,
	description,
	release_year,
...
SELECT
	'The Prestige' AS title,
	'Two rival magicians obsessively compete to create the ultimate illusion at great cost.' AS description,
...
WHERE UPPER(lang.name) = 'ENGLISH' AND
	  NOT EXISTS (
		  SELECT 1 
		  FROM public.film f
		  WHERE UPPER(f.title) = UPPER('The Prestige')
)
RETURNING film_id, title, description, release_year, language_id, rental_duration, 
		  rental_rate, length, replacement_cost, rating, last_update;
...
INSERT...SELECT ADVANTAGE:
	Even with no FK to resolve, using SELECT makes the pattern
	consistent and allows WHERE NOT EXISTS to be embedded cleanly.
...
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Jennifer') AND
		  UPPER(a.last_name) = UPPER('Garner')
)
...
SELECT
	'Mark' AS first_name,
	'Ruffalo' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Mark') AND
		  UPPER(a.last_name) = UPPER('Ruffalo')
)
RETURNING actor_id, first_name, last_name, last_update;
...
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Judy') AND
		  UPPER(a.last_name) = UPPER('Greer')
)
RETURNING actor_id, first_name, last_name, last_update;
...
SELECT
	'Emily' AS first_name,
	'Baldoni' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Emily') AND
		  UPPER(a.last_name) = UPPER('Baldoni')
)
RETURNING actor_id, first_name, last_name, last_update;
...
SELECT
	'Maury' AS first_name,
	'Sterling' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Maury') AND
		  UPPER(a.last_name) = UPPER('Sterling')
)
RETURNING actor_id, first_name, last_name, last_update;
...
SELECT
	...
	'Brendon' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Nicholas') AND
		  UPPER(a.last_name) = UPPER('Brendon')
)
RETURNING actor_id, first_name, last_name, last_update;
...
SELECT
	'Hugh' AS first_name,
	'Jackman' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Hugh') AND
		  UPPER(a.last_name) = UPPER('Jackman')
)
RETURNING actor_id, first_name, last_name, last_update;
...
SELECT
	'Christian' AS first_name,
	'Bale' AS last_name,
	CURRENT_TIMESTAMP AS last_update
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Christian') AND
		  UPPER(a.last_name) = UPPER('Bale')
)
RETURNING actor_id, first_name, last_name, last_update;
...
WHERE NOT EXISTS (
	SELECT 1
	FROM public.actor a
	WHERE UPPER(a.first_name) = UPPER('Michael') AND
		  UPPER(a.last_name) = UPPER('Caine')
)
RETURNING actor_id, first_name, last_name, last_update;
...
REFERENTIAL INTEGRITY:
	...
	tables. The WHERE clause ensures both FK values exist before the
	row is produced. CROSS JOIN simply combines the two unrelated
	tables so the filter can apply.
*/
BEGIN;
...
WHERE UPPER(a.first_name) = UPPER('Jennifer') AND
	  UPPER(a.last_name) = UPPER('Garner') AND
	  UPPER(f.title) = UPPER('13 Going on 30')
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Mark') AND
	  UPPER(a.last_name) = UPPER('Ruffalo') AND
	  UPPER(f.title) = UPPER('13 Going on 30')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Judy') AND
	  UPPER(a.last_name) = UPPER('Greer') AND
	  UPPER(f.title) = UPPER('13 Going on 30')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Emily') AND
	  UPPER(a.last_name) = UPPER('Baldoni') AND
	  UPPER(f.title) = UPPER('Coherence')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Maury') AND
	  UPPER(a.last_name) = UPPER('Sterling') AND
	  UPPER(f.title) = UPPER('Coherence')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Nicholas') AND
	  UPPER(a.last_name) = UPPER('Brendon') AND
	  UPPER(f.title) = UPPER('Coherence')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Hugh') AND
	  UPPER(a.last_name) = UPPER('Jackman') AND
	  UPPER(f.title) = UPPER('The Prestige')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Christian') AND
	  UPPER(a.last_name) = UPPER('Bale') AND
	  UPPER(f.title) = UPPER('The Prestige')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
CROSS JOIN public.film f
WHERE UPPER(a.first_name) = UPPER('Michael') AND
	  UPPER(a.last_name) = UPPER('Caine') AND
	  UPPER(f.title) = UPPER('The Prestige')
ON CONFLICT (actor_id, film_id) DO NOTHING
RETURNING actor_id, film_id, last_update;
...
REFERENTIAL INTEGRITY:
	...
	respective tables. The WHERE clause ensures both FK values exist
	before the row is produced. CROSS JOIN simply combines the two
	unrelated tables so the filter can apply.
...
SELECT 
	...
	CURRENT_TIMESTAMP AS last_update
FROM public.film f
CROSS JOIN public.category cat
WHERE UPPER(f.title) = UPPER('13 Going on 30') AND
	  UPPER(cat.name) = UPPER('Comedy')
...
FROM public.film f
CROSS JOIN public.category cat
WHERE UPPER(f.title) = UPPER('Coherence') AND
	  UPPER(cat.name) = UPPER('Sci-Fi')
ON CONFLICT (film_id, category_id) DO NOTHING
RETURNING film_id, category_id, last_update;
...
FROM public.film f
CROSS JOIN public.category cat
WHERE UPPER(f.title) = UPPER('The Prestige') AND
	  UPPER(cat.name) = UPPER('Drama')
ON CONFLICT (film_id, category_id) DO NOTHING
RETURNING film_id, category_id, last_update;
...
INSERT...SELECT ADVANTAGE:
	...
	indirectly through the address table as required. No IDs hardcoded.
 
STORE SELECTION:
	store_id is derived by joining store -> address, ordered alphabetically
	by address text (LIMIT 1).
	Assumption: the store with the alphabetically first address is chosen
	consistently on every re-run.
...
WHERE customer_id = (
	...
	WHERE UPPER(first_name) = UPPER('Tamar') AND
		  UPPER(last_name) = UPPER('Tutisani')
...
INSERT...SELECT ADVANTAGE:
	...
	dynamically via subqueries and CTEs. No IDs hardcoded anywhere.
 
CTE APPROACH (WITH ... INSERT ... RETURNING):
	Each rental INSERT uses a CTE so the RETURNING rental_id can be fed
	directly into the corresponding payment INSERT within one statement.
	This guarantees the rental_id FK in payment is always valid.
...
DUPLICATE PREVENTION:
	...
	exists, the CTE returns 0 rows, so the payment INSERT also inserts
	0 rows – fully idempotent.
...
STORE FILTER:
	Matches the same store used in Subtask 4, resolved via
	store -> address JOIN, ORDER BY address ASC, LIMIT 1.
 
STAFF SELECTION:
	MIN(staff_id) selected dynamically as required by mentor.
	Never hardcoded.
...
LIMIT 1 IN RENTAL CTE:
	...
	same store. We rent exactly one copy per transaction. The specific
	copy chosen is non-deterministic but acceptable, all copies of
	the same film are equivalent for rental purposes.
*/

BEGIN;

-- RENTAL 1 + PAYMENT 1: 13 Going on 30
WITH new_rental_13going AS (
	...
	SELECT
		'2017-01-15 10:00:00'::TIMESTAMP AS rental_date,
		i.inventory_id,
		c.customer_id,
		'2017-01-22 10:00:00'::TIMESTAMP AS return_date,
	...
	WHERE UPPER(f.title) = UPPER('13 Going on 30') AND
		  ...
		  NOT EXISTS (
			  ...
			  WHERE r.inventory_id = i.inventory_id AND
					...
					r.rental_date  = '2017-01-15 10:00:00'::TIMESTAMP
		  )
	LIMIT 1
...
-- RENTAL 2 + PAYMENT 2: Coherence
WITH new_rental_coherence AS (
	INSERT INTO public.rental (
		rental_date,
		inventory_id,
		customer_id,
	...
	SELECT
		'2017-01-16 11:00:00'::TIMESTAMP AS rental_date,
		i.inventory_id,
		c.customer_id,
		'2017-01-30 11:00:00'::TIMESTAMP AS return_date,
		(SELECT MIN(stf.staff_id) FROM public.staff stf) AS staff_id,
		CURRENT_TIMESTAMP AS last_update
	FROM public.inventory i
	INNER JOIN public.film f ON i.film_id = f.film_id 
	...
	WHERE UPPER(f.title) = UPPER('Coherence') AND
		  ...
		  NOT EXISTS (
			  ...
			  WHERE r.inventory_id = i.inventory_id AND
					r.customer_id  = c.customer_id  AND
					r.rental_date  = '2017-01-16 11:00:00'::TIMESTAMP
		  )
	LIMIT 1
	RETURNING rental_id, customer_id, inventory_id
)
...
SELECT
	...
	nr.rental_id,
	f.rental_rate AS amount,
	'2017-01-16 11:30:00'::TIMESTAMP AS payment_date
FROM new_rental_coherence nr
INNER JOIN public.inventory i ON i.inventory_id = nr.inventory_id
INNER JOIN public.film f ON f.film_id = i.film_id
RETURNING payment_id, rental_id, amount, payment_date;

-- RENTAL 3 + PAYMENT 3: The Prestige
WITH new_rental_prestige AS (
	INSERT INTO public.rental (
		rental_date,
		inventory_id,
		customer_id,
	...
	SELECT
		'2017-01-17 12:00:00'::TIMESTAMP AS rental_date,
		i.inventory_id,
		c.customer_id,
		'2017-02-07 12:00:00'::TIMESTAMP AS return_date,
		(SELECT MIN(stf.staff_id) FROM public.staff stf) AS staff_id,
		CURRENT_TIMESTAMP AS last_update
	FROM public.inventory i
	INNER JOIN public.film f ON i.film_id = f.film_id 
	...
	WHERE UPPER(f.title) = UPPER('The Prestige') AND
		  ...
		  NOT EXISTS (
			  ...
			  WHERE r.inventory_id = i.inventory_id AND
					r.customer_id  = c.customer_id  AND
					r.rental_date  = '2017-01-17 12:00:00'::TIMESTAMP
		  )
	LIMIT 1
	RETURNING rental_id, customer_id, inventory_id
)
...
SELECT
	...
	nr.rental_id,
	f.rental_rate AS amount,
	'2017-01-17 12:30:00'::TIMESTAMP AS payment_date
FROM new_rental_prestige nr
INNER JOIN public.inventory i ON i.inventory_id = nr.inventory_id
INNER JOIN public.film f ON f.film_id = i.film_id
RETURNING payment_id, rental_id, amount, payment_date;
